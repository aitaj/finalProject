...
namespace Logo.API.Controllers
{
    ...
    public class MaterialsController : ControllerBase
    {
        ...
        public MaterialsController(LogoDbContext db)
        {
            this.db = db;
        }

        [HttpGet]
        [SwaggerOperation("Butun material siyahisi")]
        public async Task<IActionResult> Get()
        {
            var data = await db.Materials.Where(s => s.DeletedDate == null).ToListAsync();
            return Ok(data);
        }

        [HttpGet("{id:int:min(1)}")]
        [SwaggerOperation("Id-ye gore bir material")]
        public async Task<IActionResult> Get(int id)
        {
            var data = await db.Materials.FirstOrDefaultAsync(s => s.Id == id && s.DeletedDate == null);
            return Ok(data);
        }
        ...
        public async Task<IActionResult> Add(Material model)
        {
            ...
            var size = await db.Materials.AddAsync(model);
        ...
        public async Task<IActionResult> Edit(int id, Material model)
        {
            ...
            var entity = await db.Materials.FirstOrDefaultAsync(s => s.Id == id && s.DeletedDate == null);
